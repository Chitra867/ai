...
router.get('/generate', authenticate, async (req: AuthRequest, res) => {
  try {
    ...
    const threatAnalyses = await ThreatAnalysis.find({
      userId,
      createdAt: { $gte: startDate }
    }).sort({ createdAt: -1 });

    // Get malware scan data
    const malwareScans = await MalwareScan.find({
      userId,
      createdAt: { $gte: startDate }
    }).sort({ createdAt: -1 });
    ...
    const malwareStats = {
      ...
      malwareTypes: {} as { [key: string]: number },
      severityBreakdown: {
        critical: 0,
        high: 0,
        medium: 0,
...
router.get('/templates', authenticate, (req: AuthRequest, res) => {
  const templates = [
    {
      id: 'comprehensive',
...
router.get('/history', authenticate, async (req: AuthRequest, res) => {
  try {
    ...
    const recentAnalyses = await ThreatAnalysis.find({ userId })
      .sort({ createdAt: -1 })
      .limit(limit)
...
// Helper functions
function calculateOverallRiskLevel(threatStats: any, malwareStats: any): string {
...
function generateKeyFindings(threatStats: any, malwareStats: any): string[] {
  const findings = [];
...
function generateRecommendations(threatStats: any, malwareStats: any): Array<{priority: string, category: string, recommendation: string}> {
