...
router.get('/overview', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const threatStats = await Promise.all([
            ThreatAnalysis_1.default.countDocuments({ userId, createdAt: { $gte: last24Hours } }),
            ThreatAnalysis_1.default.countDocuments({ userId, createdAt: { $gte: last7Days } }),
            ThreatAnalysis_1.default.countDocuments({ userId, createdAt: { $gte: last30Days } }),
            ThreatAnalysis_1.default.countDocuments({
                userId,
        ...
        const malwareStats = await Promise.all([
            MalwareScan_1.default.countDocuments({ userId, createdAt: { $gte: last24Hours } }),
            MalwareScan_1.default.countDocuments({ userId, createdAt: { $gte: last7Days } }),
            MalwareScan_1.default.countDocuments({ userId, createdAt: { $gte: last30Days } }),
            MalwareScan_1.default.countDocuments({
                userId,
        ...
        const recentAlerts = await ThreatAnalysis_1.default.find({
            userId,
            'threats.severity': { $in: ['high', 'critical'] },
        ...
        res.json({
            ...
            recentAlerts: recentAlerts.flatMap(analysis => analysis.threats
...
router.get('/threat-trends', auth_1.authenticate, async (req, res) => {
    try {
        const userId = req.user._id;
        const days = parseInt(req.query.days) || 30;
        const startDate = new Date(Date.now() - days * 24 * 60 * 60 * 1000);
        // Aggregate threat data by day
        const threatTrends = await ThreatAnalysis_1.default.aggregate([
            {
                $match: {
                    userId,
...
router.get('/malware-stats', auth_1.authenticate, async (req, res) => {
    try {
        const userId = req.user._id;
        const days = parseInt(req.query.days) || 30;
        const startDate = new Date(Date.now() - days * 24 * 60 * 60 * 1000);
        ...
        const scanTrends = await MalwareScan_1.default.aggregate([
            {
                $match: {
                    userId,
                    createdAt: { $gte: startDate }
...
router.get('/activity-logs', auth_1.authenticate, async (req, res) => {
    try {
        const userId = req.user._id;
        ...
        const threatLogs = await ThreatAnalysis_1.default.find({ userId })
            .sort({ createdAt: -1 })
            .skip(skip)
            .limit(limit / 2)
            .select('analysisType status summary createdAt');
        // Get recent malware scans
        const malwareLogs = await MalwareScan_1.default.find({ userId })
            .sort({ createdAt: -1 })
            .skip(skip)
            .limit(limit / 2)
